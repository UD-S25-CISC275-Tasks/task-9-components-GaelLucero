...
export function toMarkdown(question: Question): string {
    let message: string = "# " + question.name + "\n" + question.body;
    if (question.type === "multiple_choice_question") {
        let options = question.options.reduce((list: string, option: string) => list + "\n- " + option, "")
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    let copyQuestion = {...oldQuestion, id: id, name: "Copy of " + oldQuestion.name, published: false}
    return copyQuestion;
}
...
export function addOption(question: Question, newOption: string): Question {
    let copyQuestion = {...question, options: [...question.options, newOption]}
    return copyQuestion;
}
...
export function mergeQuestion(
    ...
    let newQuestion = {...contentQuestion, id: id, name: name, points: points, published: false}
    return newQuestion;
}
